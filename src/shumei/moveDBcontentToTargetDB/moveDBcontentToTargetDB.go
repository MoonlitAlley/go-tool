...
func scanAndMove() error {
	...
	*/
	...
	for rows.Next() {
		...
		var insertSql = "insert into `sentry_rule_engine_variable` " +
			"(`variableId`, `variableName`, `definition`, `class`, `type`, `organization`,`appId`, `enabled`, `eventId`, `statisticModel`, `definitionSource`) " +
			"values (?,?,?,?,?,?,?,?,?,?,?) on duplicate key update `enabled` = VALUES(enabled);"

		sqlPrepare, prepareErr := targetDB.Prepare(insertSql)
		if prepareErr != nil {
			fmt.Println(fmt.Sprintf("Mysql Prepare error(%v)    sql = (%s)", prepareErr, insertSql))
			return nil
		}

		for i := 0; i < 3; i++ {
			_, execErr := sqlPrepare.Exec(
				variableId,
				variableName,
				definition,
				*class,
				...
				definitionSource)
			if execErr != nil {
				fmt.Print(fmt.Sprintf("Mysql Exec error(%v)", execErr))
				continue
			}
			break
		}
		count++
		if err := sqlPrepare.Close(); err != nil {
			log.Logger.LogThirdPartFailP(log.LL_ERROR, log.LT_UNEXCEPTED, "", "", fmt.Sprintf("Mysql Prepare close error(%v)", err), 0)
		}
	}
	fmt.Print(fmt.Sprintf("move count = %v", count))
	return nil
}
