...
type PredictRequest struct {
	...
	EventId      *string `thrift:"eventId,9" json:"eventId,omitempty"`
}
...
var PredictRequest_EventId_DEFAULT string

func (p *PredictRequest) GetEventId() string {
	if !p.IsSetEventId() {
		return PredictRequest_EventId_DEFAULT
	}
	return *p.EventId
}
...
func (p *PredictRequest) IsSetEventId() bool {
...
func (p *PredictRequest) readField9(iprot thrift.TProtocol) error {
	if v, err := iprot.ReadString(); err != nil {
...
func (p *PredictRequest) writeField9(oprot thrift.TProtocol) (err error) {
	if p.IsSetEventId() {
		if err := oprot.WriteFieldBegin("eventId", thrift.STRING, 9); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T write field begin error 9:eventId: ", p), err)
		}
		if err := oprot.WriteString(string(*p.EventId)); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T.eventId (9) field write error: ", p), err)
		}
		if err := oprot.WriteFieldEnd(); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T write field end error 9:eventId: ", p), err)
		}
	}
	return err
}
